...
if __name__ == "__main__":
    ...
    init_batch_size = 200                       # The number of evaluations of the initial batch ('batch' = population)
    ...
    mutation_pb = 0.1                           # The probability of mutating each value of a genome
    eta = 10.0                                  # The ETA parameter of the polynomial mutation (as defined in the origin NSGA-II paper by Deb.). It corresponds to the crowding degree of the mutation. A high ETA will produce mutants close to its parent, a small ETA will produce offspring with more changes.
    ...
    # Create container
    container = NoveltyArchive(k=k, threshold_novelty=threshold_novelty, fitness_domain=fitness_domain, features_domain=features_domain, storage_type=list)

    # Define evaluation function
    def eval_fn(ind):
        #Evaluation function takes an individual as input, and returns the pair ``(fitness, features)``
        sim = cannon_physics.FIT_QD()
        sim.eval(ind)
