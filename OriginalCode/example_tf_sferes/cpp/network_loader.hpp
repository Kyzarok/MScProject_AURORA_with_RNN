...
class NetworkLoader{
  ...
  void prepare_batches(std::vector<std::vector<std::pair<std::string, Tensor> > >& batches,const Mat& data) const
  {
    if(data.rows()<=Options::batch_size)
      ...
      batches=std::vector<std::vector<std::pair<std::string, Tensor> > >(floor(data.rows()/(Options::batch_size)));
  ...
  float training(const Mat& data, bool full_train=false) {
    ...
    for(size_t repl = 0; repl <5; repl++){
      ...
      std::cout<<"INIT train loss: "<<init_tr_recon_loss<<"  INIT validation loss: "<<init_vl_recon_loss<<std::endl;
	...
	if(!full_train && epoch%100 == 0)
	  {
	    ...
	    if((previous_avg.array()-previous_avg[0]).mean() > 0 && get_avg_recon_loss(train_db)<init_tr_recon_loss)
	      _continue=false;
	  }
	if(epoch%1000 == 0)
	  std::cout<<epoch<<"  valid: "<< get_avg_recon_loss(valid_db)<< " train: "<< get_avg_recon_loss(train_db)<<std::endl;
	
	

	
      }
      std::cout<<repl<<" Final train loss: "<<get_avg_recon_loss(train_db)<<"  Final validation loss: "<<get_avg_recon_loss(valid_db)<<std::endl;
    }
  ...
  void eval(const Mat& data, Mat& desc, Mat& recon_loss, Mat& loss, Mat& reconst) const {
    ...
    std::vector<Tensor> outputs;
    Status status = _session->Run({eval_data}, {"encoder/latent","loss","recon_loss","decoder/decoded","learning_rate"},{},  &outputs);
    if (!status.ok()) {
      LOG(ERROR) << status;
    }
